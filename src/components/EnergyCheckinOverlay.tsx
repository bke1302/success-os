    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,.85)', padding: '0 16px 32px' }}>
      <div className="w-full animate-slide-up" style={{ maxWidth: 420, background: '#000', border: '1px solid rgba(255,255,255,.09)', borderRadius: 20, borderTop: `3px solid ${color}`, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 20px 16px', borderBottom: '1px solid rgba(255,255,255,.09)' }}>
          ...
          <button onClick={onClose} aria-label="סגור צ׳ק-אין אנרגיה" className="btn-ghost" style={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <X className="w-4 h-4" />
          </button>
        </div>
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <input type="range" min={1} max={10} value={score} onChange={e => setScore(Number(e.target.value))} className="w-full" style={{ accentColor: color }} aria-label="בחר רמת אנרגיה 1 עד 10" aria-valuemin={1} aria-valuemax={10} aria-valuenow={score} />
            ...
            {[
              ...
              return (
                <div key={opt.label} onClick={() => setScore(Math.round((opt.range[0]+opt.range[1])/2))}
                  role="button" tabIndex={0} aria-label={`בחר ${opt.label}`}
                  onKeyDown={e => e.key === 'Enter' && setScore(Math.round((opt.range[0]+opt.range[1])/2))}
          ...
          <button onClick={handleSave} aria-label={`שמור צ׳ק-אין אנרגיה — ציון ${score}`} className="btn-red w-full"
